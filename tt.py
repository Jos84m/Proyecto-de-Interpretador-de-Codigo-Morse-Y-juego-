...
class Ventana_Principal(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("STRANGER TEC")
        self.geometry("900x560")
        self.resizable(False, False)
        self.iconbitmap("icono.ico")  
        self.configure(bg="#0A0E1A")
        self.configure(relief=tk.GROOVE, bd=30, bg="#000000")
...
#================================================================================================================
    def _build_splash(self):
        self.canvas = tk.Canvas(self, width=900, height=560,
                                bd=0, highlightthickness=0,
                                bg="#0A0E1A")
        self.canvas.pack(fill=tk.BOTH, expand=True)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        # Carga de frames del fondo del GIF animado
        GIF_PATH = os.path.join(script_dir, "mmm.gif")
        self.gif_frames = []
        self.gif_index  = 0
        ...
        try:
            ...
            while True:
                try:
                    img.seek(frame_num)
                    frame_resized = img.copy().resize((900, 560), Image.LANCZOS)
...
#================================================================================================================
# Funciones de animación del GIF animado.
#================================================================================================================
...
class MorseGame(tk.Tk):
    def __init__(self):
        ...
        # Control para animación idle de LEDs
        self._led_idle_running = True
        self._build_fonts()
        self._build_ui()
...
#================================================================================================================
# Pestaña JUEGO — ahora incluye panel de maqueta integrado
#================================================================================================================
    def _build_tab_juego(self):
        ...
        self.lbl_frase.pack(pady=6)

        self.lbl_morse_frase = tk.Label(frase_card, text="",
                                        font=self.font_mono, bg=BG_CARD, fg=TEXT_SEC)
        self.lbl_morse_frase.pack(pady=(0,6))

        # ── Contenedor principal: jugadores + maqueta ──
        main_area = tk.Frame(p, bg=BG_DARK)
        main_area.pack(fill=tk.BOTH, expand=True, padx=10, pady=4)

        # ── Panel jugadores (izquierda) ──
        jugadores_frame = tk.Frame(main_area, bg=BG_DARK)
        jugadores_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)

        card_a = self._card(jugadores_frame, "JUGADOR A  (Teclado)")
        card_a.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(0,4))
        self._build_player_panel(card_a, "A")

        card_b = self._card(jugadores_frame, "JUGADOR B  (Maqueta/Botones)")
        card_b.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(4,0))
        self._build_player_panel(card_b, "B")

        # ── Panel Maqueta integrado (derecha) ──
        maqueta_lateral = self._card(main_area, "MAQUETA · LEDs")
        maqueta_lateral.pack(side=tk.LEFT, fill=tk.Y, padx=(8,0))

        # Grid de 16 LEDs compacto (2 filas × 8 columnas)
        led_grid_j = tk.Frame(maqueta_lateral, bg=BG_CARD)
        led_grid_j.pack(padx=8, pady=(6,2))

        # Lista separada de LEDs para la vista de JUEGO (no interfiere con la pestaña MAQUETA)
        self.led_juego_list = []
        for i in range(16):
            row, col = divmod(i, 8)
            c = tk.Canvas(led_grid_j, width=30, height=30,
                          bg=BG_CARD, highlightthickness=0)
            c.grid(row=row, column=col, padx=3, pady=3)
            oval = c.create_oval(3, 3, 27, 27, fill=LED_OFF, outline="#2A3A4A", width=1)
            num  = c.create_text(15, 15, text=str(i+1),
                                 font=self.font_small, fill="#3A4A5A")
            self.led_juego_list.append((c, oval, num))

        # Número representado
        self.lbl_num_led_j = tk.Label(maqueta_lateral, text="—",
                                       font=self.font_sub, bg=BG_CARD, fg=LED_ON)
        self.lbl_num_led_j.pack(pady=(2,0))

        tk.Label(maqueta_lateral, text="Núm. representado",
                 font=self.font_small, bg=BG_CARD, fg=TEXT_SEC).pack()

        # Canvas visual morse compacto
        tk.Label(maqueta_lateral, text="Señal Morse:",
                 font=self.font_small, bg=BG_CARD, fg=TEXT_SEC).pack(pady=(8,0))

        self.morse_canvas_j = tk.Canvas(maqueta_lateral, width=270, height=36,
                                         bg="#080D18", highlightthickness=0)
        self.morse_canvas_j.pack(padx=8, pady=(2,4))

        # Botones jugador B en panel lateral
        tk.Label(maqueta_lateral, text="Botones Jugador B:",
                 font=self.font_small, bg=BG_CARD, fg=TEXT_SEC).pack(pady=(4,2))

        btns_lateral = tk.Frame(maqueta_lateral, bg=BG_CARD)
        btns_lateral.pack(pady=2, padx=6)

        tk.Button(btns_lateral, text="●\nPUNTO", font=self.font_small,
                  bg=DOT_COLOR, fg=BG_DARK, relief=tk.FLAT,
                  width=6, height=2,
                  command=lambda: self._sim_boton(".")).grid(row=0, column=0, padx=2, pady=2)

        tk.Button(btns_lateral, text="━\nRAYA", font=self.font_small,
                  bg=DASH_COLOR, fg=BG_DARK, relief=tk.FLAT,
                  width=6, height=2,
                  command=lambda: self._sim_boton("-")).grid(row=0, column=1, padx=2, pady=2)

        tk.Button(btns_lateral, text="□\nESP", font=self.font_small,
                  bg="#5A6A7A", fg=TEXT_PRI, relief=tk.FLAT,
                  width=6, height=2,
                  command=lambda: self._sim_boton(" ")).grid(row=1, column=0, padx=2, pady=2)

        tk.Button(btns_lateral, text="/\nPAL", font=self.font_small,
                  bg="#3A4A5A", fg=TEXT_PRI, relief=tk.FLAT,
                  width=6, height=2,
                  command=lambda: self._sim_boton("/")).grid(row=1, column=1, padx=2, pady=2)

        tk.Button(btns_lateral, text="⌫\nBORRAR", font=self.font_small,
                  bg="#4A1515", fg=RED_COLOR, relief=tk.FLAT,
                  width=13, height=2,
                  command=self._sim_borrar).grid(row=2, column=0, columnspan=2, padx=2, pady=2)

        # ── Botones acción ──
        btn_row = tk.Frame(p, bg=BG_DARK)
        btn_row.pack(pady=6)
        self._neon_btn(btn_row, " NUEVA FRASE",     ACCENT1,     self._nueva_frase).pack(side=tk.LEFT, padx=6)
        self._neon_btn(btn_row, " REPRODUCIR MORSE",ACCENT3,     self._reproducir_morse).pack(side=tk.LEFT, padx=6)
        self._neon_btn(btn_row, " EVALUAR RONDA",   GREEN_COLOR, self._evaluar_ronda).pack(side=tk.LEFT, padx=6)
        self._neon_btn(btn_row, " REINICIAR",       ACCENT2,     self._reiniciar).pack(side=tk.LEFT, padx=6)
...
#================================================================================================================
# Pestaña MAQUETA (independiente, sin cambios)
#================================================================================================================
    ...
    def _build_tab_referencia(self):
        ...
        for i, (ch, code) in enumerate(digits):
            ...
            dots_frame.pack()
            self._render_morse_dots(dots_frame, code)
            # CORRECCIÓN: faltaban los paréntesis () en .pack()
            tk.Label(cell, text=code, font=self.font_small,
                     bg=BG_CARD, fg=TEXT_SEC).pack()
    ...
    def _play_morse_thread(self):
        ...
        # Actualizar ambas etiquetas de morse visual (pestaña MAQUETA y pestaña JUEGO)
        ...
        self.morse_canvas_j.delete("all")
        ...
        for sym in symbols:
            ...
            for i, ch in enumerate(sym):
                if ch == '.':
                    ...
                    self.morse_canvas_j.create_rectangle(x, 8, x+w, 28,
                                                          fill=DOT_COLOR, outline="")
                    self._led_on()
                    time.sleep(unit)
                    self._led_off()
    ...
    def _led_on(self):
        """Enciende LEDs en AMBOS paneles (maqueta independiente + juego)."""
        for c, oval, num in self.led_canvas_list:
            c.itemconfig(oval, fill=LED_ON)
        for c, oval, num in self.led_juego_list:
            c.itemconfig(oval, fill=LED_ON)

    def _led_off(self):
        """Apaga LEDs en AMBOS paneles."""
        for c, oval, num in self.led_canvas_list:
    ...
    def _animate_leds_idle(self):
        """
        Animación idle de LEDs en AMBAS listas.
        CORRECCIÓN: usa self.after en lugar de recursión infinita sin control,
        y verifica que la ventana siga existiendo antes de continuar.
        """
        self._led_idle_counter = getattr(self, "_led_idle_counter", 0)

        def tick():
            try:
                idx = self._led_idle_counter
                for i, (c, oval, num) in enumerate(self.led_canvas_list):
                    dist = abs(i - idx % 16)
                    c.itemconfig(oval, fill=ACCENT1 if dist < 2 else LED_OFF)
                for i, (c, oval, num) in enumerate(self.led_juego_list):
                    dist = abs(i - idx % 16)
                    c.itemconfig(oval, fill=ACCENT1 if dist < 2 else LED_OFF)
                self._led_idle_counter += 1
                self.after(80, tick)
            except tk.TclError:
                # La ventana fue destruida; detener la animación limpiamente
                pass
