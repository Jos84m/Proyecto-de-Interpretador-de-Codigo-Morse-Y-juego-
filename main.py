...
import time                                        # Utilizamos time para manejar los tiempos de las señales morse y animaciones
import threading                                   # Utilizamos threading para manejar tareas en segundo plano como la animación de LEDs o la lectura de la maqueta sin bloquear la interfaz
...
class Ventana_Principal(tk.Tk):
    def __init__(self):                                             # Inicialización de la ventana de bienvenida con sus características
        super().__init__()                                          # Llama al constructor de la clase base tk.Tk para inicializar la ventana de bienvenida
        self.title("STRANGER TEC")                                  # Título de la ventana de bienvenida
        self.geometry("900x560")                                    # Tamaño inicial de la ventana de bienvenida
        self.resizable(False, False)                                # Evita que el usuario pueda redimensionar la ventana de bienvenida
        self.iconbitmap("icono.ico")                                # Establece el ícono de la ventana de bienvenida 
        self.configure(bg="#000000")                              # Configura el fondo de la ventana de bienvenida con un color oscuro para resaltar el contenido y darle un aspecto moderno y tecnológico.
        self.configure(relief=tk.GROOVE, bd=30, bg="#000000")     # Agrega un borde decorativo a la ventana de bienvenida para darle un aspecto más definido y elegante
        self.update_idletasks()                                     # Asegura que se actualicen las dimensiones de la ventana antes de centrarla
        x = (self.winfo_screenwidth()  // 2) - (900 // 2)           # Calcula la posición x para centrar la ventana de bienvenida en la pantalla
        y = (self.winfo_screenheight() // 2) - (560 // 2)           # Calcula la posición y para centrar la ventana de bienvenida en la pantalla
        self.geometry(f"900x560+{x}+{y}")                           # Establece la geometría de la ventana de bienvenida con el tamaño y la posición calculados para centrarla en la pantalla
        self._build_splash()                                        # Llama a la función que construye la pantalla de bienvenida con su diseño, animaciones y elementos gráficos
#================================================================================================================
# Funcion de creacion de la pantalla de bienvenida.
#================================================================================================================
    def _build_splash(self):                                             # Construye la pantalla de bienvenida con su diseño, animaciones y elementos gráficos
        self.canvas = tk.Canvas(self, width=900, height=560,             # Crea un canvas para dibujar el fondo, texto y elementos gráficos de la pantalla de bienvenida
                                bd=0, highlightthickness=0,              # Elimina bordes y resaltados del canvas para que se integre mejor con el diseño general de la pantalla de bienvenida
...
# Carga de frames del foindo del GIF animado                       ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        try:                                                                    
            ...
            frame_num = 0                                          # Inicializa el índice del frame en 0
            while True:                                            # Itera para cargar todos los frames del GIF animado de fondo, escalándolos al tamaño de la ventana de bienvenida
                try:
                    img.seek(frame_num)                                            # Selecciona el frame actual del GIF animado de fondo utilizando Pillow
                                                                                   # Escalar al tamaño de la ventana
                    frame_resized = img.copy().resize((900, 560), Image.LANCZOS)   # Escala el frame actual del GIF animado de fondo al tamaño de la ventana de bienvenida utilizando un filtro de alta calidad para mantener la claridad visual
                    self.gif_frames.append(ImageTk.PhotoImage(frame_resized))      # Agrega el frame escalado a la lista de frames del GIF animado de fondo para su uso en la animación de la pantalla de bienvenida
                    frame_num += 1                                                 # Incrementa el índice del frame para cargar el siguiente frame del GIF animado de fondo en la próxima iteración
                except EOFError:                                                   # Cuando se alcanza el final del GIF animado de fondo, se lanza una excepción EOFError
                    break                                                          # sale del bucle de carga de frames del GIF animado de fondo
        except ImportError:                                                        # Si Pillow no está disponible, intenta cargar el GIF animado de fondo con Tkinter nativo, 
                                                                                   # Sin Pillow: cargar con tkinter nativo
            try:
                frame_num = 0                                                      # Índice para controlar el frame actual del GIF animado de fondo durante la carga utilizando Tkinter nativo
                while True:                                                        # Itera para cargar todos los frames del GIF animado de fondo utilizando Tkinter nativo
                    try:
                        frame = tk.PhotoImage(file=GIF_PATH, format=f"gif -index {frame_num}")     # Carga el frame actual del GIF animado de fondo utilizando Tkinter nativo
                        self.gif_frames.append(frame)                                              # Agrega el frame cargado a la lista de frames del GIF animado de fondo para su uso en la animación de la pantalla de bienvenida
                        frame_num += 1                                                             # Incrementa el índice del frame para cargar el siguiente frame del GIF animado de fondo en la próxima iteración
                    except tk.TclError:                                                            # Cuando se alcanza el final del GIF animado de fondo, se lanza una excepción tk.TclError
                        break
        ...
        if self.gif_frames:
            self.gif_item = self.canvas.create_image(0, 0, anchor=tk.NW,
...
# Subtitulo de nuestro juego en la pantalla de bienvenida -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        self.canvas.create_text(450, 245,
                                text="Juego de Código Morse · 2 Jugadores",
                                font=("Courier New", 13),
                                fill="#8892A4",
                                anchor=tk.CENTER)
...
# Colocación del botón en el canvas-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        btn_window = self.canvas.create_window(450, 400,
                                               anchor=tk.CENTER,
                                               window=btn_frame)
# Botón Iniciar Juego ademas con sus respectivas caracteristcas y efetos------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
# Efecto hover para el botón-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        btn.bind("<Enter>", lambda e: btn.config(bg="#00FFD1", fg="#0A0E1A"))
        btn.bind("<Leave>", lambda e: btn.config(bg="#111827", fg="#00FFD1"))
# Información adicional en la parte inferior de la pantalla de bienvenida ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        self.canvas.create_text(450, 500,
                                text="v1.0  ·  Raspberry Pi Pico W  ·  Python / Tkinter",
                                font=("Courier New", 9),
                                fill="#3A4A5A",
                                anchor=tk.CENTER)
# Texto morse decorativo animado en la parte inferior-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
        self.morse_text = self.canvas.create_text(450, 530,
                                                   text="",
                                                   font=("Courier New", 10),
                                                   fill="#1E4A3A",
                                                   anchor=tk.CENTER)
        self._animate_morse_text()
#=============================================================================================================================================================================================================================================
# Funciones de animación para usar un fondo de formato GIF animado.
#============================================================================================================================================================================================================================================
...
#=============================================================================================================================================================================================================================================
    def _draw_fallback_bg(self):
        colors = ["#0A0E1A", "#0C1120", "#0E1426", "#101728", "#0C1120"]
        step = 560 // len(colors)
        for i, color in enumerate(colors):
            self.canvas.create_rectangle(0, i * step, 900, (i+1) * step,
                                         fill=color, outline="")
        rnd.seed(42)
        for _ in range(80):
            x = rnd.randint(0, 900)
            y = rnd.randint(0, 560)
            r = rnd.randint(1, 3)
            alpha_colors = ["#0D2020", "#0F2525", "#112828", "#0B1E1E"]
            self.canvas.create_oval(x-r, y-r, x+r, y+r,
                                    fill=rnd.choice(alpha_colors), outline="")
#=============================================================================================================================================================================================================================================
# Funcion de animación para el texto decorativo de código morse en la ventana principal
#=============================================================================================================================================================================================================================================
    def _animate_morse_text(self):
        ...
        self._morse_anim_idx = idx + 1
        self.after(1800, self._animate_morse_text)
#=============================================================================================================================================================================================================================================
...
class MorseGame(tk.Tk):
    def __init__(self):
        ...
        self.velocidad    = tk.StringVar(value="A")          
        self.turno        = 1                               
...
# Función para crear un "card" o panel con un título, esta función se utiliza para crear secciones visuales en la interfaz con un fondo oscuro y un borde de acento, además de mostrar un título en la parte superior del panel para identificar su contenido------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    def _neon_btn(self, parent, text, color, command):
        ...
        return btn
# =============================================================================================================================================================================================================================================================================================
# Función principal — ahora lanza el SplashScreen primero
# =============================================================================================================================================================================================================================================================================================
if __name__ == "__main__":
    splash = Ventana_Principal()
    splash.mainloop()